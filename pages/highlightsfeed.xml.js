...
export async function getServerSideProps({ res }) {
  ...
  const articlesResponse = await fetch(`https://readwise.io/api/v2/books/?category=articles`, headers);

  const articles = await articlesResponse.json();

  res.setHeader('Content-Type', 'text/xml');
  res.setHeader('Cache-Control', `s-maxage=${60 * 60 * 2}, stale-while-revalidate`);
